...
Alpaca leverages Scala 3's powerful pattern matching system to provide a type-safe and intuitive way to define grammar rules. "Extractors" are the mechanism used to match terminals (tokens) and non-terminals (rules) within a `rule` block.

## 1. Matching Terminals (Tokens)

When you define a lexer, Alpaca automatically generates extractors for each token. You can use these extractors to match tokens and access their values.
...
Rules defined with `val myRule = rule(...)` also act as extractors. When matched, they return the value produced by that rule.
