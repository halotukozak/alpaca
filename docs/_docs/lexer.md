...
The Lexer in Alpaca is responsible for transforming a raw string of characters into a stream of structured tokens. It uses a powerful DSL based on Scala 3 macros to define lexical rules using regular expressions.

## Defining a Lexer

A lexer is defined using the `lexer` block. Within this block, you use pattern matching where the patterns are regular expression strings and the results are token definitions.
...
- `Token.Ignored`: Creates a token that is matched but excluded from the final token stream (useful for whitespace and comments).
...
The Lexer can maintain state during tokenization using a `LexerCtx`. This is useful for tracking line numbers, character positions, or handling nested structures.
...
The `tokenize` method returns a tuple containing the final state of the context and a list of `Lexeme` objects. Each `Lexeme` contains the token name, the extracted value, and the context state at the time of matching.
...
3. Generates an efficient DFA-based scanner.
