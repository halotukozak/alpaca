\chapter{Cel pracy i~wizja projektu}
\label{ch:cel-wizja}
...
Analizatory leksykalne (\textit{lexers}) i~składniowe (\textit{parsers}) stanowią fundamentalne komponenty procesu kompilacji, realizując odpowiednio fazę analizy leksykalnej i~syntaktycznej~\cite{aho2006}.
Analizator leksykalny transformuje ciąg znaków wejściowych w~strumień tokenów (leksemów) zgodnie z~regułami języka regularnego~\cite{hopcroft2006}, podczas gdy analizator składniowy weryfikuje zgodność strumienia tokenów z~gramatyką bezkontekstową języka, konstruując drzewo składni abstrakcyjnej (AST, \textit{Abstract Syntax Tree})~\cite{aho2006}.
...
Wyrażenia regularne stanowią notację deklaratywną dla języków regularnych i~mogą być mechanicznie przekształcone w~automaty skończone za pomocą konstrukcji Thompsona~\cite{thompson1968}.
Automaty deterministyczne (DFA) gwarantują liniową złożoność czasową rozpoznawania~$O(n)$, podczas gdy niedeterministyczne (NFA) mogą wymagać przeszukiwania z~nawrotami (\textit{backtracking}).

Gramatyki bezkontekstowe (CFG) definiują strukturę syntaktyczną języków programowania.
Parsery dla CFG dzielą się na dwie główne kategorie: parsery zstępujące (\textit{top-down}), takie jak LL($k$)~\cite{lewis1968}, oraz parsery wstępujące (\textit{bottom-up}), takie jak LR($k$)~\cite{knuth1965}.
...
Niniejsza praca stawia tezę, że wykorzystanie metaprogramowania w~języku Scala~3 (makra kompilacyjne, typy rafinowane) umożliwia konstrukcję systemu generującego leksery i~parsery o~następujących właściwościach:

\begin{enumerate}
    \item \textbf{Wydajność} --- czas parsowania porównywalny z~narzędziami opartymi na generacji kodu (ANTLR, Yacc), przewyższający biblioteki interpretowane (PLY, SLY).
    \item \textbf{Użyteczność} --- interfejs programistyczny (API) niezależny od dedykowanego DSL, zintegrowany z~systemem typów Scali i~wspierany przez standardowe narzędzia IDE\@.
    \item \textbf{Diagnostyka błędów} --- komunikaty błędów generowane w~czasie kompilacji (dla błędów gramatyki) oraz w~czasie parsowania (dla błędów składniowych), zawierające kontekst syntaktyczny.
\end{enumerate}

Celem pracy inżynierskiej jest stworzenie narzędzia \textit{ALPACA} (\textit{Another Lexer Parser And Compiler Alpaca}) w~języku Scala, które implementuje funkcjonalności powszechnie stosowane w~budowie lekserów i~parserów, weryfikując postawioną tezę.
...
\subsection{Luka w~dostępnych rozwiązaniach}
\label{subsec:luka-badawcza}

Analiza literatury oraz istniejących implementacji (sekcja~\ref{sec:przeglad-istniejacych-rozwiazan}) ujawniła następujące problemy:

\paragraph{Kompromis między wydajnością a~użytecznością}
Narzędzia oparte na generacji kodu (Yacc, ANTLR) osiągają wysoką wydajność, ale wymagają nauki dedykowanego języka dziedzinowego (DSL) oraz dodatkowego kroku kompilacji.
Biblioteki kombinatorów parserów (FastParse, parboiled2) oferują interfejs zintegrowany z~językiem gospodarza, ale kosztem spadku wydajności związanej z~interpretacją reguł w~czasie wykonania~\cite{fastparse-docs}.

\paragraph{Niska jakość komunikatów błędów}
Systemy generujące parsery tabelowe (Yacc, Bison) produkują komunikaty błędów o~niskiej wartości diagnostycznej, często ograniczające się do ogólnego komunikatu \enquote{syntax error}.
Przyczyna tkwi w~braku kontekstu syntaktycznego dostępnego w~automatach LR(1) po wykryciu błędu.

\paragraph{Brak integracji z~nowoczesnymi środowiskami IDE}
Narzędzia historyczne (Lex, Yacc) nie oferują wsparcia dla współczesnych środowisk programistycznych, co utrudnia debugowanie i~analizę gramatyk.
Nowe rozwiązania (ANTLR) wymagają dedykowanych pluginów, co ogranicza ich adopcję.

\subsection{Cele projektu}
\label{subsec:cele-projektu}

Projekt~\textit{ALPACA} ma na celu eliminację wymienionych ograniczeń poprzez:

\begin{enumerate}
    \item \textbf{Połączenie wydajności generatorów kodu z~użytecznością bibliotek} --- wykorzystanie makr kompilacyjnych Scali~3 pozwala przenieść część obliczeń na etap kompilacji, zachowując interfejs programistyczny zintegrowany z~systemem typów języka.

    \item \textbf{Generowanie komunikatów błędów w~oparciu o~kontekst parsera~LR(1)} --- parser ma dostęp do pełnej informacji o~stanach automatu, co umożliwia identyfikację oczekiwanych terminali i~generowanie sugestii naprawy.

    \item \textbf{Natywna integracja ze środowiskami IDE} --- implementacja w~czystym języku Scala eliminuje konieczność stosowania dedykowanych pluginów, wykorzystując istniejące wsparcie dla języka (IntelliJ IDEA, Metals).
\end{enumerate}
...
Proces ten odbywa się przed kompilacją programu głównego i~wymaga dodatkowego narzędzia w~łańcuchu budowania (\textit{build chain}).

\subsubsection{Lex i~Yacc}
...
Narzędzia te wymagają znajomości dedykowanej składni specyfikacji gramatyk, co podnosi próg wejścia dla początkujących użytkowników.
...
\lstinputlisting[language=Python,caption={Fragment definicji parsera w~Pythonie, wykorzystujący bibliotekę SLY},label={lst:python-parser}]{listings/chapter1/sly-parser.py}

Komunikaty błędów w~bibliotece \textit{SLY} są bardzo ograniczone, co obrazuje przykład~\ref{lst:not-working-sly}, który po uruchomieniu informuje użytkownika błędem z~fragmentu kodu~\ref{lst:sly-result}.
Okazuje się, że problemem był brak atrybutu \texttt{ignore\_comment} w~definicji \texttt{Lexer}.

\lstinputlisting[language=Python,caption={Fragment niedziałającego kodu w~Pythonie, wykorzystujący bibliotekę SLY},label={lst:not-working-sly}]{listings/chapter1/not-working-sly.py}
...
Biblioteka \textit{Scala parser combinators}~\cite{moors2008parser} była popularnym sposobem na tworzenie parserów, lecz jak wynika z~dokumentacji: \enquote{Trudno jest jednak zrozumieć ich działanie i~jak zacząć.
...
Niestety próg wejścia ze względu na skomplikowany i~nieintuicyjny DSL jest wysoki.
...
Biblioteki kombinatorów (FastParse, parboiled2) oferują interfejs zintegrowany z~językiem gospodarza, ale kosztem spadku wydajności związanej z~interpretacją reguł w~czasie wykonania.
...
\begin{table}[ht]
    ...
    \begin{tabular}{L|CCCC}
        ...
        \hline
        Wydajność               & wysoka                                    & umiarkowana          & wysoka             & ---                  \\
        \hline
        Łatwość użycia          & niska                                     & średnia              & średnia            & ---                  \\
        \hline
        Aktywne wsparcie        & nie                                       & nie                  & tak                & ---                  \\
        \hline
        Diagnostyka błędów      & dobra                                     & niska                & dobra              & ---                  \\
        \hline
        Dokumentacja            & słaba                                     & bardzo dobra         & bardzo dobra       & ---                  \\
        \hline
        Popularność             & średnia                                   & niska                & rosnąca            & ---                  \\
        \hline
        Integracja IDE          & wsparcie dla Scali                        & wsparcie dla Scali   & wsparcie dla Scali & ---                  \\
        \hline
        Wsparcie do debugowania & dobre                                     & dobre                & dobre              & ---                  \\
        \hline
        Generowanie kodu        & nie                                       & nie                  & nie                & ---                  \\
        \bottomrule
    \end{tabular}
...
\section{Ograniczenia i~zakres pracy}
...
\begin{itemize}
    \item \textbf{Optymalizacje stanów parsera:} System generuje kanoniczne stany LR(1) bez minimalizacji do LALR(1), co może prowadzić do większych tablic akcji.
    Implementacja minimalizacji stanowi kierunek przyszłych badań.

    \item \textbf{Obsługa gramatyk niejednoznacznych:} Parser wykrywa konflikty shift/reduce i~reduce/reduce, ale nie oferuje mechanizmu automatycznego rozwiązywania niejednoznaczności (poza priorytetami operatorów).

    \item \textbf{Ewaluacja empiryczna w~kontekście dydaktycznym:} Weryfikacja użyteczności systemu w~środowisku akademickim (badanie z~udziałem studentów) wykracza poza zakres pracy i~stanowi kierunek przyszłych badań.
\end{itemize}
