...
\subsubsection{Regularne spotkania}
...
\noindent Trudność w~diagnozowaniu tego typu błędów wynika z~następujących czynników:
...
\noindent Notacja \texttt{X before Y} oznacza, że~operator \texttt{X} ma~wyższy priorytet niż~operator~\texttt{Y}.

\noindent System automatycznie:
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        \hline
        \multirow{4}{*}{Lekser}    & Testowanie definicji tokenów (nazwy, wzorce, wartości domyślne).                            \\ \cline{2-2}
                                   & Testowanie ignorowanych reguł leksykalnych.                                                 \\ \cline{2-2}
                                   & Testowanie wykrywania konfliktów nazw tokenów (błędy kompilacji).                           \\ \cline{2-2}
                                   & Testowanie obsługi niepoprawnych wyrażeń regularnych.                                       \\
        \hline
        \multirow{4}{*}{Parser}    & Testowanie ekstrakcji produkcji z~definicji.                                                \\ \cline{2-2}
                                   & Testowanie algorytmów LR(1) (\emph{closure}, \emph{goto}, obliczanie zbiorów \emph{FIRST}). \\ \cline{2-2}
                                   & Testowanie rozwiązywania konfliktów typu \emph{shift--reduce}.                              \\ \cline{2-2}
                                   & Testowanie akcji semantycznych z~różnymi typami kontekstu.                                  \\
        \hline
        \multirow{2}{*}{Funkcje pomocnicze}
                                   & Testowanie derywacji (Empty[T], Copyable[T]).                                 \\ \cline{2-2}
                                   & Testowanie mechanizmu withDefault[T].                                              \\ \cline{2-2}
        \hline
    \end{tabularx}
...
\noindent System weryfikowany był na~trzech poziomach złożoności:

\begin{itemize}
    \item Test podstawowy --- proste wyrażenie $1 + 2$ zwracające oczekiwany wynik $3.0$;

    \item Test złożoności średniej --- wielopoziomowe nawiasowanie i~operacje łączące operatory o~różnych priorytetach,
          takie jak~wyrażenie $((12 + 7) \cdot (3 - 8 / (4 + 2)) + (15 - (9 - 3 \cdot (2 + 1))) / 5) \dots$,
    ...
    \item Test zaawansowany --- wyrażenie zawierające funkcje trygonometryczne,
          operatory potęgowania i~dzielenia całkowitego, takie jak~$\sin(\pi/6) + \cos(\pi/3) + (2^{3^2}) / (3 + 1) + \dots$,
...
\noindent System weryfikowany był na~czterech poziomach złożoności:

\begin{itemize}
    \item Test primitywnych wartości --- proste wyrażenie $\texttt{true}$ zwracające wartość logiczną;
