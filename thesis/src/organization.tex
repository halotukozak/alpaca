...
\subsubsection[README i wiki]{README i~wiki}

\begin{itemize}

    \item \textbf{README.md} --- instrukcje instalacji, przykłady użycia, architektura systemu.

    \item \textbf{GitHub Wiki} --- dokumentacja bardziej szczegółowa, poradniki, FAQ.

\end{itemize}

\subsubsection{Rozprawy dyplomowa (LaTeX)}
...
\begin{itemize}
    ...
    \item Bibliografia w~BibTeX (\verb|bibliografia.bib|) z~odwołaniami do~papieru naukowych i~dokumentacji.
...
\begin{itemize}
    ...
    \item Dla modułów o~jasno zdefiniowanych specyfikacjach (np. funkcja \verb|computeFirstSet|) testy były napisywane przed implementacją.

    \item Dla modułów eksperymentalnych (np. system generacji klas w~makrach) testy były pisane po~implementacji.

    \item Praktyka przydatna do~ujawniania problemów w~interfejsach między modułami.
