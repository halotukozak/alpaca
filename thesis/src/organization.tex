...
Rozdział przedstawia metodykę realizacji projektu dyplomowego \textit{ALPACA}, jego charakter, podział obowiązków pomiędzy~członkami zespołu, organizację prac oraz zastosowane techniki inżynierskie.
...
\label{subsec:osoby-w-projekcie}

Projekt realizowany był przez zespół dwóch osób o~jasno zdefiniowanych rolach.

\paragraph[Bartosz Buczek (dalej: BB)]{Bartosz~Buczek (dalej: BB)} był odpowiedzialny przede wszystkim za implementację algorytmów analizy leksykalnej i~składniowej, opracowanie mechanizmu rozwiązywania konfliktów gramatycznych oraz przygotowanie testów wydajnościowych.

\paragraph[Bartłomiej Kozak (dalej: BK)]{Bartłomiej~Kozak (dalej: BK)} odpowiadał za implementację języka dziedzinowego (DSL) systemu \textit{ALPACA} z~wykorzystaniem metaprogramowania w~Scali~3, generację tabel parsowania w~fazie kompilacji, projektowanie zaawansowanych mechanizmów systemu typów oraz opracowanie dokumentacji.
...
\begin{itemize}
    \item Moduł \texttt{alpaca.parser}
    \item Klasy: \texttt{Parser[Ctx]}, \texttt{Rule[R]}, \texttt{ParseTable}, \texttt{ActionTable}
    \item Makra: \texttt{createTables}
\end{itemize}
...
W~przypadku zagadnień o~szczególnie złożonym charakterze zastosowano sesje wspólnego programowania (ang. \textit{Pair Programming}).
...
Podczas cyklicznych spotkań omawiano priorytety oraz wybierano kolejne zgłoszenia (ang. \textit{issues}) do~realizacji.
W procesie planowania wykorzystywano elementy techniki Planning Game (Gra Planistyczna), ustalając zakres prac w oparciu o szacowaną złożoność i wartość poszczególnych zadań.
...
\subsubsection{Obliczenia wieloetapowe (ang. \textit{Staged Computation})}
...
Zastosowano reguły ochrony gałęzi (ang. \textit{branch protection rules}), wymagające pozytywnego wyniku wszystkich testów przed połączeniem zmian z~głównym branchem.
...
Tabela~\ref{tab:detailed-timeline} prezentuje podział prac na~etapy wraz z~kluczowymi osiągnięciami i~liczbą zmian wprowadzonych w~każdym okresie.
...
\begin{itemize}
    ...
    \item \textbf{Rozmiar implementacji} wynosi około \textbf{4000 linii kodu implementacyjnego} (bez uwzględnienia testów) oraz około \textbf{2000 linii kodów testowych}.
\end{itemize}

Rozkład pracy w~czasie pokazany na rysunku~\ref{fig:commits} ilustruje dynamikę zespołu, wskazując na stały postęp oraz okresy intensywnej pracy nad rozwiązywaniem kluczowych wyzwań technicznych.
...
Osiągnięto pokrycie testami na~poziomie około TODO kodu, przy czym objęto nimi główne ścieżki wykonania oraz przypadki brzegowe.
