...
Rozdział przedstawia metodykę realizacji projektu dyplomowego \textit{ALPACA}, charakter projektu, podział obowiązków pomiędzy członkami zespołu, organizację prac oraz zastosowane techniki inżynierskie.
...
\subsection{Typ projektu}
\label{subsec:typ-projektu}

Projekt \textit{ALPACA} ma charakter \textbf{badawczo-rozwojowy} i~łączy elementy badań teoretycznych z~praktyczną implementacją.
...
\paragraph{Bartosz~Buczek (dalej: BB)} pełnił rolę współautora projektu.
Odpowiadał przede wszystkim za implementację algorytmów analizy leksykalnej i~składniowej, opracowanie mechanizmu rozwiązywania konfliktów gramatycznych oraz przygotowanie testów wydajnościowych.

\paragraph{Bartłomiej~Kozak (dalej: BK)} pełnił rolę współautora projektu.
Był odpowiedzialny za implementację języka dziedzinowego (DSL) systemu \textit{ALPACA} z~wykorzystaniem metaprogramowania w~Scali~3, generację tabel parsowania w~fazie kompilacji, projektowanie zaawansowanych mechanizmów systemu typów oraz opracowanie dokumentacji.


\subsection{Podział prac na główne zadania}
\label{subsec:podzial-prac-na-glowne-zadania}
...
\begin{itemize}
    ...
    \item Deklaratywne rozwiązywanie konfliktów \textit{shift--reduce} i~\textit{reduce--reduce}.\ (BB, BK)
...
\label{subsec:wspolpraca-miedzy-czlonkami-zespolu}
...
Zadania rejestrowano w~postaci zgłoszeń \textit{GitHub Issues} i~organizowano na~tablicy Kanban\cite{alpaca-project}, a~kamienie milowe wraz z~terminami realizacji definiowano i~monitorowano z~wykorzystaniem mechanizmu GitHub Milestones\cite{alpaca-milestones}.
...
\label{subsec:komunikacja-zespoowa}
...
\begin{itemize}
    \item \textbf{GitHub Issues i~Pull Requests} --- główne narzędzie do~dyskusji nad kodem, proponowania zmian oraz rejestrowania błędów.
    \item \textbf{Signal} --- kanał komunikacji tekstowej dla szybkich pytań oraz wymiany odnośników i~materiałów.
    \item \textbf{Spotkania osobiste} --- wykorzystywane przede wszystkim do~omówień strategicznych i~podejmowania decyzji architektonicznych.
\end{itemize}

\subsubsection{Ustalanie kamieni milowych}

Podział prac na etapy został sformalizowany poprzez zdefiniowanie pięciu kamieni milowych (ang. \textit{milestones}), każdego powiązanego z~wyraźnym terminem i~katalogiem zadań.
...
Każdy kamień milowy zawierał listę zgłoszeń (ang. \textit{issues}) reprezentujących konkretne zadania (implementacja funkcji, dokumentacja, testy).
...
Strukturę kamieni milowych oraz postępy realizacji przedstawiono w~tabel \ref{tab:milestones-progress}.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{|l|X|c|c|}
        ...
        \hline
    \end{tabularx}
    \caption{Kamienie milowe projektu \textit{ALPACA} i~postęp ich realizacji}
    \label{tab:milestones-progress}
\end{table}
...
\label{subsec:narzedzia-programistyczne-ici/cd}
...
Wykorzystano framework testowy \textbf{ScalaTest}, który umożliwił przygotowanie testów jednostkowych dla modułów leksera i~parsera, a~także testów integracyjnych dla pełnych przykładów (parser wyrażeń arytmetycznych, parser uproszczonego formatu JSON). Testy uruchamiano poleceniem \verb|mill test|.
...
\subsection{Dokumentacja}\label{subsec:dokumentacja}
...
\subsubsection[README i dokumentacja]{README i~dokumentacja}

\paragraph{README.md} zawiera instrukcje instalacji, przykłady użycia oraz zarys architektury systemu.
\paragraph{GitHub Pages} hostuje bardziej szczegółową dokumentację, poradniki oraz sekcję FAQ\cite{alpaca-docs}.
...
\label{subsec:metodologia-wytwarzania-oprogramowania}
...
\begin{enumerate}
    \item \textbf{Prototypowanie} --- szybkie eksperymentowanie z~różnymi podejściami.
    \item \textbf{Ewaluacja} --- ocena wydajności, przydatności oraz spójności otrzymanych rozwiązań.
    \item \textbf{Udoskonalanie} --- ulepszanie wybranych podejść.
    \item \textbf{Integracja} --- łączenie poszczególnych komponentów w~spójny system.
\end{enumerate}
...
W~ramach wybranych zmian każdy pull request zawierał sekcję poświęconą refaktoryzacji, jeżeli była ona uzasadniona.
...
Każdy pull request był recenzowany przez drugiego członka zespołu przed połączeniem z~główną gałęzią.
Stosowano Pair Programming, czyli sesje wspólnego programowania, w~przypadku szczególnie złożonych problemów.
Spotkania poświęcone przeglądom kodu odbywały się 2--3 razy w~tygodniu, zwykle przez 30--60~minut.

\subsubsection[Issues i Planning Game]{User Stories i~Planning Game}
Podczas cyklicznych spotkań omawiano priorytety oraz wybierano kolejne \textit{issues} do~realizacji.
Backlog utrzymywano w~postaci zgłoszeń \textit{GitHub Issues}

\subsection{Praktyki specyficzne dla metaprogramowania}
\label{subsec:praktyki-specyficzne-dla-metaprogramowania}
...
Stosowano zasadę szybkiego zwrotu błędu (ang. \textit{fail-fast}) --- preferowano odrzucanie niepoprawnych danych na~etapie kompilacji zamiast zgłaszania błędów dopiero w~czasie wykonania.

\subsection{Praktyki DevOps}
\label{subsec:praktyki-devops}
...
Zastosowano reguły ochrony gałęzi (branch protection rules), wymagające pozytywnego wyniku wszystkich testów przed połączeniem zmian z~główną gałęzią.

\subsubsection{Zarządzanie artefaktami (Artifact Management)}

Wersje rozwojowe (SNAPSHOT) i stabilne wydania są publikowano w~repozytorium Maven Central.
...
\label{subsec:szczegolowa-os-czasu}

Projekt ALPACA realizowany był przez okres 27~tygodni, obejmujących dwa semestry akademickie. Szczegółowy przebieg prac oraz liczba zmian wprowadzonych w~poszczególnych etapach zostały udokumentowane poprzez system kontroli wersji Git, umożliwiający śledzenie postępów projektu na~poziomie pojedynczych zmian.

W~toku realizacji projektu zebrano 216~commitów (zmian w~kodzie), z~których średnio przypadało 19,6~zmian na~tydzień. Poniższa tabela prezentuje podział prac na~etapy wraz z~kluczowymi osiągnięciami i~liczbą zmian wprowadzonych w~każdym okresie.
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{|l|l|X|}
        ...
        \hline
        \multicolumn{3}{c}{\textbf{\textit{Semestr 1 --- Eksploracja i~prototypowanie (tygodnie 1--14)}}}                  \\
        \hline
        ...
        \hline
        \multicolumn{3}{c}{\textbf{\textit{Semestr 2 --- Implementacja i~optymalizacja (tygodnie 15--27)}}}                \\
        \hline
...
\label{subsec:velocitas-zespou-ipostepy}

\subsubsection{Metryki wydajności}

Realizacja projektu ALPACA wymagała znaczącego zaangażowania zespołu przez okres kilku miesięcy. Skala pracy oraz złożoność napotkanych wyzwań technicznych znajdują odzwierciedlenie w poniższych wskaźnikach:

\begin{itemize}
    \item \textbf{Historię rozwoju reprezentuje} łącznie \textbf{450 commitów} zintegrowanych w głównej gałęzi projektu, dokumentujących iteracyjny proces implementacji oraz poprawiania systemu.
    \item \textbf{Struktura zmian kodowych} obejmuje około \textbf{80 pull requestów}, z których każdy zawierał recenzję i testy weryfikujące poprawność dodawanych funkcjonalności.
    \item \textbf{Rozmiar implementacji} wynosi około \textbf{4000 linii kodu implementacyjnego} (bez uwzględnienia testów) oraz około \textbf{2000 linii kodów testowych}, co świadczy o wysokim pokryciu testami wynoszącym 33%.
\end{itemize}

Rozkład pracy w~czasie pokazany na rysunku~\ref{fig:commits} ilustruje dynamikę zespołu, wskazując na konsystentny postęp oraz okresy intensywnej pracy nad resolucją kluczowych wyzwań technicznych.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/Commits.png}
...
\subsection{Problem 1: Limit rozmiaru metody JVM}
\label{subsec:problem-1:-limit-rozmiaru-metody-jvm}
...
Każdy wpis tabeli parsowania (para: stan, symbol terminalny $\rightarrow$ akcja \textit{shift}/\textit{reduce}) wymagał wygenerowania kilku instrukcji.
Dla złożonych gramatyk z~tysiącami wpisów rozmiar wygenerowanej metody przekraczał dopuszczalny limit.

\subsubsection{Rozwiązanie}

Zastosowano podejście polegające na~fragmentacji metod (\textit{method fragmentation}) \cite{method-too-large}.
...
\subsection{Problem 2: Transmisja referencji między etapami kompilacji}
\label{subsec:problem-2:-transmisja-referencji-miedzy-etapami-kompilacji}
...
Zaimplementowano klasę \verb|ReplaceRefs| realizującą transformację AST poprzez zmianę przypisania symboli (ang.~\textit{re-owning}). Transformacja przebiega według następujących kroków:
...
\subsection[Problem 3: Typy rafinowane a wsparcie IDE]{Problem 3: Typy rafinowane a~wsparcie IDE}
\label{subsec:problem-3:-typ-rafinowanie-aide-support}

\subsubsection{Problem}

W wersji pozbawionej typów rafinowanych interfejs API nie dostarczał środowisku IDE informacji o~dostępnych tokenach.
Funkcje autouzupełniania, podpowiedzi typów oraz mechanizmy \textit{go-to-definition} działały w~sposób ograniczony.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/IDESupport.png}
    \caption{Wsparcie IDE dzięki typom rafinowanym w~projekcie ALPACA}
    \label{fig:ide_support}
\end{figure}
...
\subsection{Problem 4: Wydajność kompilacji makr}
\label{subsec:problem-4:-wydajnosc-kompilacji-makr}
...
Algorytmy \textit{Closure} i~\textit{Goto} dla LR(1) wymagają wielu iteracji oraz porównań zbiorów stanów.
...
\begin{enumerate}
    ...
    \item Wprowadzono buforowanie wyników pośrednich (m.in.\ zbiorów \textit{FIRST}).
...
\subsection[Problem 5: Testowanie makr i metaprogramowania]{Problem 5: Testowanie makr i~metaprogramowania}
\label{subsec:problem-5:-testowanie-makr-imetaprogramowania}
...
Osiągnięto pokrycie testami na~poziomie około 70\% kodu, przy czym objęto nimi główne ścieżki wykonania oraz przypadki brzegowe.

\subsection{Problem 6: Błędy w~kolejności definicji tokenów}
\label{subsec:problem-6:-nieprawidlowa-kolejnosc-definicji-regexow}

\subsubsection{Problem}

Użytkownicy systemu ALPACA mogą intuicyjnie zdefiniować tokeny w~kolejności, która prowadzi do~nieoczekiwanych błędów.
Problem występuje, gdy~krótszy wzorzec jest~zdefiniowany przed dłuższym wariantem, który zawiera go~jako prefiks.

Przykładowo, użytkownik definiuje operatory porównania w~naturalnej kolejności:
\begin{verbatim}
LT = <
LE = <=
GT = >
...
Taka definicja wydaje się poprawna, jednak podczas parsowania wyrażenia \verb|x <= 5| system rozpozna tylko \verb|<|, a~znak \verb|=| zostanie odrzucony jako nieznany token, co~prowadzi do~błędu składniowego.

\subsubsection{Przyczyna}

Ze~względu na~wykorzystanie bibliotecznych wyrażeń regularnych (szczegóły w~rozdziale~\ref{sec:dfa-vs-regex}), system dopasowuje tokeny według kolejności definicji, a~nie według reguły najdłuższego dopasowania.
Problem jest szczególnie trudny do znalezienia, ponieważ:
\begin{itemize}
    \item Błąd nie~objawia się podczas definiowania leksera (kompilacja przebiega poprawnie)
    \item Objawy pojawiają się dopiero podczas parsowania konkretnego wejścia
    \item Komunikat błędu odnosi się do~parsera (\enquote{nieoczekiwany symbol}), nie~do~leksera
\end{itemize}

\subsubsection{Rozwiązanie}

System automatycznie wykrywa takie konflikty podczas kompilacji dzięki modułowi \verb|RegexChecker| (algorytm opisany w~rozdziale~\ref{subsec:lexer-impl-regexchecker}).
Przed wygenerowaniem kodu leksera, wszystkie wzorce są~analizowane pod~kątem konfliktów, a~użytkownik otrzymuje czytelny komunikat błędu wskazujący źródło problemu.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/PatternShadowing.png}
    \caption{Przykład wykrycia konfliktu w~kolejności definicji tokenów}
    \label{fig:regex_checker_error}
\end{figure}

\subsubsection{Rezultat}

Błędy są wykrywane w~fazie kompilacji, zanim użytkownik uruchomi kod.
Komunikat diagnostyczny jasno wskazuje, które wzorce kolidują, umożliwiając natychmiastową korektę.
Użytkownik musi jedynie zmienić kolejność definicji (najpierw \verb|<=|, potem \verb|<|) lub~zmodyfikować wyrażenia regularne, aby~wyeliminować konflikt.

Mechanizm ten znacząco skrócił czas debugowania błędów leksykalnych podczas rozwoju projektów demonstracyjnych (parser wyrażeń arytmetycznych, parser JSON).

\subsection{Problem 7: Konflikty precedencji operatorów}
\label{subsec:problem-7:-konflikty-w-tabeli-shift-reduce}

\subsubsection{Problem}

Użytkownicy definiujący gramatyki dla~wyrażeń arytmetycznych napotykają problem: jak~powiedzieć parserowi, że~mnożenie ma~wyższy priorytet niż~dodawanie?

Bez~jawnych reguł precedencji, parser nie~wie, czy~wyrażenie \verb|2 + 3 * 4| powinno być parsowane jako~\verb|(2 + 3) * 4 = 20| czy~\verb|2 + (3 * 4) = 14|.
System zgłasza błąd kompilacji wskazujący na~konflikt, wymagając od~użytkownika rozstrzygnięcia.

Dodatkowo, użytkownicy mogą przypadkowo zdefiniować sprzeczne reguły precedencji.
Na~przykład, próbując zdefiniować unary minus, użytkownik może przez pomyłkę użyć zwykłego operatora odejmowania, tworząc cykliczną zależność: operator~$-$ ma~pierwszeństwo przed~$*$, $*$ przed~$-$, co~jest niemożliwe do~rozstrzygnięcia.
...
Bez mechanizmu wykrywania i~rozwiązywania tych konfliktów, użytkownik byłby zmuszony do przepisania gramatyki do~formy jednoznacznej (np.~różne nieterminale dla~każdego poziomu precedencji), co~jest uciążliwe i~zmniejsza czytelność specyfikacji.

\subsubsection{Rozwiązanie}

System ALPACA oferuje mechanizm deklaratywnego rozwiązywania konfliktów bez~konieczności przepisywania gramatyki (algorytmy opisane w~rozdziale~\ref{subsec:parser-conflict-resolution}).

\paragraph{Prosty przykład użycia}
...
\begin{enumerate}
    \item \textbf{Wyprowadza przechodniość}: Skoro \verb|pow| przed \verb|mul| i~\verb|mul| przed \verb|add|, to~\verb|pow| przed \verb|add|
    \item \textbf{Rozwiązuje wszystkie konflikty}: Używając grafu relacji pierwszeństwa
    \item \textbf{Weryfikuje spójność}: Sprawdza czy~nie~ma~cykli w~relacjach
\end{enumerate}


Jeśli~użytkownik przez pomyłkę zdefiniuje sprzeczne reguły (np.~$A$ przed~$B$, $B$ przed~$C$, $C$ przed~$A$), system wykrywa cykl podczas kompilacji i~zgłasza czytelny błąd:
...
\begin{itemize}
    \item Definicja gramatyki w~naturalnej formie, bez~sztucznych przekształceń
    \item Redukcja liczby wymaganych deklaracji z~$O(n^2)$ do~$O(n)$ w~typowych przypadkach
    \item Wczesna walidacja konfliktów i~niespójności wykrywana w~fazie kompilacji
    \item Pomocne komunikaty wskazujące źródło problemu i~sugestie rozwiązania
\end{itemize}

Mechanizm ten umożliwił implementację przykładowego parsera wyrażeń arytmetycznych z~pełną obsługą operatorów \verb|+|, \verb|-|, \verb|*|, \verb|/|, \verb|^| oraz~nawiasów w~mniej niż~40~liniach kodu.


\section[Wdrożenia, testy i~eksperymenty]{Opis wdrożeń, testów i~eksperymentów}
\label{sec:wdrozenia-testy}

\subsection{Testowanie jednostkowe}
\label{subsec:testowanie-jednostkowe}

Projekt zawiera rozbudowany zestaw testów jednostkowych, obejmujących poszczególne moduły systemu.

\subsubsection{Testy modułu leksera}
...
\begin{itemize}
    ...
    \item Testowanie algorytmów LR(1) (\textit{closure}, \textit{goto}, obliczanie zbiorów \textit{FIRST}).
    \item Testowanie rozwiązywania konfliktów typu \textit{shift--reduce}.
...
\label{subsec:testowanie-integracyjne}
...
Zaimplementowano parser wyrażeń arytmetycznych z~operatorami \verb|+|, \verb|-|, \verb|*|, \verb|/| oraz zagnieżdżonymi nawiasami.
Akcje semantyczne obliczają wartość analizowanego wyrażenia.

\begin{itemize}
    \item Przykładowe wejście: \verb|3 + 4 * 2|
    \item Oczekiwany rezultat: \verb|11|
\end{itemize}

\subsubsection{Parser JSON (uproszczony)}

Zaimplementowano parser dla podzbioru formatu JSON (obsługujący obiekty, tablice, napisy, liczby, wartości logiczne i~\verb|null|).

\begin{itemize}
    \item Przykładowe wejście: \verb|{"name": "Alice", "age": 30}|
    \item Rezultat: struktura danych reprezentująca obiekt JSON\@.
\end{itemize}

\subsection{Benchmarki wydajności}
\label{subsec:benchmarki-wydajnosci}

Przeprowadzono benchmarki porównujące wydajność wygenerowanego parsera \textit{ALPACA} z~innymi podejściami.

TODO: to zmyślone


\subsection{Walidacja poprawności}
\label{subsec:walidacja-poprawnosci}
