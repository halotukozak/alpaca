...
Cytaty (ang.~\emph{quotes}), oznaczane jako \verb|'{...}|, służą do opóźnienia wykonania kodu i~traktowania go jako danych~\cite{scala3-reflection}.
Wstawki (ang.~\emph{splices}), oznaczane jako \verb|${...}|, pozwalają na ocenę wyrażenia generującego kod i~wstawienie wyniku do otaczającego kontekstu~\cite{scala3-reflection,scala3-guides-quotes}.

Formalna semantyka tych konstrukcji została przedstawiona w~pracy Stuckiego, Brachthäusera i~Odersky'ego~\cite{stucki2021multistage}, gdzie cytaty (ang.~\emph{quotes}) i~wstawki (ang.~\emph{splices}) są traktowane jako prymitywne formy w~typowanych drzewach składniowych (ang.~\emph{typed abstract syntax trees}).
...
\subsection{Dopasowanie wzorców w~cytatach kodu}\label{subsec:dopasowanie-wzorcow-kodu}
