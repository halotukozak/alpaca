...
\label{ch:analiza-porownawcza}


\section{Wprowadzenie do badań porównawczych}
\label{sec:wprowadzenie-do-badan}

Weryfikacja empiryczna tezy pracy wymaga systematycznego porównania systemu~\textit{ALPACA} z~reprezentatywnymi rozwiązaniami istniejącymi na~rynku.
...
Wybór narzędzi porównawczych wynikał z~analizy przeprowadzonej w~sekcji~\ref{sec:przeglad-istniejacych-rozwiazan}.
Jako reprezentantów głównych kategorii rozwiązań wybrano:

\begin{itemize}
    \item \textbf{SLY}~\cite{sly} --- bibliotekę interpretowaną dla języka Python, reprezentującą podejście oparte na~refleksji i~dynamicznym typowaniu (kategoria bibliotek interpretowanych),
...
Pominięto generatory kodu takie jak ANTLR ze~względu na~fundamentalne różnice w~architekturze (zewnętrzny krok generacji vs.~metaprogramowanie w~czasie kompilacji), które utrudniają bezpośrednie porównanie wydajności.


\section{Metodologia badań}
\label{sec:metodologia-badan}
...
\label{subsec:charakterystyka-danych-testowych}
...
\label{subsubsec:dane-iteracyjne}
...
\lstinputlisting[language=terminal,caption={Struktura iteracyjnych danych testowych dla wyrażeń arytmetycznych},label={lst:iterative-math-structure}]{listings/comparison/iterative-math-structure.txt}

Dla formatu JSON dane iteracyjne reprezentują tablicę obiektów o~stałej głębokości zagnieżdżenia:

\lstinputlisting[language=json,caption={Struktura iteracyjnych danych testowych dla JSON},label={lst:iterative-json-structure}]{listings/comparison/iterative-json-structure.json}

\subsubsection{Dane rekurencyjne}
\label{subsubsec:dane-rekurencyjne}
...
\lstinputlisting[language=terminal,caption={Struktura rekurencyjnych danych testowych dla wyrażeń arytmetycznych},label={lst:recursive-math-structure}]{listings/comparison/recursive-math-structure.txt}

Dla formatu JSON dane rekurencyjne reprezentują głęboko zagnieżdżone obiekty:

\lstinputlisting[language=json,caption={Struktura rekurencyjnych danych testowych dla JSON},label={lst:recursive-json-structure}]{listings/comparison/recursive-json-structure.json}

\subsection{Scenariusze testowe}
\label{subsec:scenariusze-testowe}
...
\label{sec:implementacja-parserow-testowych}
...
\subsection{Implementacja w~ALPACA}
\label{subsec:implementacja-alpaca}

Parser wyrażeń arytmetycznych w~systemie~\textit{ALPACA} wykorzystuje deklaratywny interfejs DSL oparty na~dopasowaniu wzorców:

\lstinputlisting[language=scala,caption={Parser wyrażeń arytmetycznych w~systemie ALPACA},label={lst:alpaca-math-parser},escapeinside={(*@}{@*)}]{listings/comparison/alpaca-math-parser.scala}

\subsection{Implementacja w~SLY}
\label{subsec:implementacja-sly}

Odpowiadająca implementacja w~bibliotece SLY wykorzystuje dekoratory i~refleksję nazw metod:

\lstinputlisting[language=python,caption={Parser wyrażeń arytmetycznych w~bibliotece SLY},label={lst:sly-math-parser}]{listings/comparison/sly-math-parser.py}

\subsection{Implementacja w~FastParse}
\label{subsec:implementacja-fastparse}

FastParse realizuje parsowanie poprzez kompozycję funkcji parserowych:

\lstinputlisting[language=scala,caption={Parser wyrażeń arytmetycznych w~bibliotece FastParse},label={lst:fastparse-math-parser}]{listings/comparison/fastparse-math-parser.scala}
...
\label{sec:wyniki-badan}

\subsection{Wyrażenia arytmetyczne --- dane iteracyjne}
\label{subsec:wyniki-iterative-math}

Tabela~\ref{tab:iterative-math-results} przedstawia wyniki testów wydajnościowych dla wyrażeń arytmetycznych o~strukturze iteracyjnej.
...
\begin{table}[ht]
    ...
    \label{tab:iterative-math-results}
\end{table}

\subsection{Wyrażenia arytmetyczne --- dane rekurencyjne}
\label{subsec:wyniki-recursive-math}

Tabela~\ref{tab:recursive-math-results} przedstawia wyniki dla wyrażeń arytmetycznych o~strukturze rekurencyjnej (głębokie zagnieżdżenie nawiasów).
...
\begin{table}[ht]
    ...
    \begin{tabular}{l|rrr|rrr|r}
        ...
        2000    & 220,96 ms & 25,82 ms & 274,95 ms & 43,06 ms & 150,68 ms & 194,77 ms & \textit{Stack\-Over\-flow} \\
        \bottomrule
    \end{tabular}
    \caption{Wyniki testów wydajnościowych dla wyrażeń arytmetycznych (dane rekurencyjne)}
    \label{tab:recursive-math-results}
\end{table}
...
\label{subsec:wyniki-iterative-json}

Tabela~\ref{tab:iterative-json-results} przedstawia wyniki dla formatu JSON o~strukturze iteracyjnej (tablica obiektów).
...
\begin{table}[ht]
    ...
    \label{tab:iterative-json-results}
\end{table}

\subsection{Format JSON --- dane rekurencyjne}
\label{subsec:wyniki-recursive-json}

Tabela~\ref{tab:recursive-json-results} przedstawia wyniki dla formatu JSON o~strukturze rekurencyjnej (głęboko zagnieżdżone obiekty).
...
\begin{table}[ht]
    ...
    \label{tab:recursive-json-results}
\end{table}

Wyniki dla rekurencyjnych danych JSON ujawniają problem wydajnościowy w~module leksykalizacji systemu~\textit{ALPACA} dla danych o~wysokim stopniu zagnieżdżenia.
Czas leksykalizacji rośnie nieproporcjonalnie do~rozmiaru danych, co~wymaga dalszej analizy i~optymalizacji (sekcja~\ref{subsec:wnioski-alpaca}).
...
Lekser systemu~\textit{ALPACA} osiąga porównywalną wydajność dla danych iteracyjnych, jednak wykazuje znaczący spadek wydajności dla danych rekurencyjnych JSON\@.
...
System~\textit{ALPACA} tokenizuje każdą sekwencję białych znaków jako osobny token typu \verb|Ignored|, co~wymaga dopasowania wzorca wyrażenia regularnego dla każdej spacji.

Biblioteka SLY stosuje odmienne podejście --- pomijanie ignorowanych znaków realizowane jest prostym warunkiem iteracyjnym:

\lstinputlisting[language=python,caption={Mechanizm pomijania białych znaków w~bibliotece SLY},label={lst:sly-ignore-mechanism}]{listings/comparison/sly-ignore-mechanism.py}
...
Optymalizacja mechanizmu ignorowania znaków stanowi kierunek przyszłych prac opisanych w~sekcji~\ref{sec:kierunki-dalszych-prac}.

\subsubsection{Faza parsowania}

Faza parsowania systemu~\textit{ALPACA} wykazuje stabilną wydajność, znacząco przewyższającą bibliotekę SLY dla wszystkich testowanych scenariuszy.
Różnica ta wynika z~fundamentalnych różnic architektonicznych:
...
\label{subsec:zachowanie-zagniezdzone}
...
Przeprowadzone eksperymenty pozwalają na sformułowanie następujących wniosków dotyczących wydajności systemu \textit{ALPACA}.

\textbf{Mocne strony systemu:}
\begin{enumerate}
    \item Faza parsowania systemu \textit{ALPACA} charakteryzuje się stabilną i przewidywalną wydajnością, przewyższającą bibliotekę \textit{SLY} we wszystkich analizowanych scenariuszach testowych.
...
\textbf{Obszary wymagające optymalizacji:}
...
\label{sec:porownanie-api}

Oprócz wydajności, istotnym kryterium oceny narzędzi jest jakość interfejsu programistycznego (API).
Tabela~\ref{tab:comparison} zestawia kluczowe aspekty interfejsów porównywanych narzędzi.
...
\label{subsec:bezpieczenstwo-typow-api}

System~\textit{ALPACA} oferuje pełne bezpieczeństwo typów na~poziomie kompilacji dzięki wykorzystaniu typów rafinowanych (sekcja~\ref{subsec:typy-rafinowane}).
...
Fragment kodu~\ref{lst:sly-type-error} ilustruje sytuację, w~której błąd typowania w~akcji semantycznej zostanie wykryty dopiero podczas parsowania konkretnego wejścia:

\lstinputlisting[language=python,caption={Błąd typowania wykrywany dopiero w~czasie wykonania (SLY)},label={lst:sly-type-error}]{listings/comparison/sly-type-error.py}

\subsection{Integracja ze~środowiskiem IDE}
\label{subsec:integracja-ide}

System~\textit{ALPACA} oferuje pełną integrację ze~standardowymi narzędziami IDE dla języka Scala (IntelliJ IDEA, Metals) bez konieczności instalacji dedykowanych wtyczek.
Funkcjonalności takie jak:

\begin{itemize}
    \item automatyczne uzupełnianie nazw tokenów,
    \item nawigacja do~definicji (\textit{go-to-definition}),
...
Analizatory statyczne, takie jak mypy, generują liczne błędy dla poprawnego kodu SLY, co~ilustruje fragment~\ref{lst:mypy-analysis}:

\lstinputlisting[language=terminal,caption={Wynik analizy statycznej mypy dla parsera SLY},label={lst:mypy-analysis}]{listings/comparison/mypy-math-parser-analysis.txt}
...
Rysunek~\ref{fig:sly-ide-warnings} przedstawia widok kodu SLY w~środowisku VS Code, gdzie widoczne są ostrzeżenia generowane przez wbudowany analizator statyczny:

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/SlyMathParserInIDE.png}
    \caption{Ostrzeżenia IDE dla poprawnego kodu parsera SLY w~środowisku PyCharm}
    \label{fig:sly-ide-warnings}
\end{figure}


\section{Podsumowanie analizy porównawczej}
\label{sec:podsumowanie-analizy}

Przeprowadzona analiza porównawcza pozwala na~weryfikację tezy postawionej w~rozdziale~\ref{ch:cel-wizja}.
W~odniesieniu do~pytań badawczych sformułowanych w~sekcji~\ref{sec:teza-badawcza}:

\begin{enumerate}
    \item Faza parsowania systemu~\textit{ALPACA} osiąga wydajność porównywalną z~FastParse i~znacząco przewyższa bibliotekę SLY\@.
...
\label{sec:kierunki-dalszych-prac}

Na~podstawie przeprowadzonych badań zidentyfikowano następujące kierunki dalszego rozwoju systemu~\textit{ALPACA}:
