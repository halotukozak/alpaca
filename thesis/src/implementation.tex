...
Kluczowym mechanizmem implementacyjnym makra \texttt{lexer} jest programatyczna konstrukcja klasy anonimowej w~czasie kompilacji.
Proces ten wykorzystuje API refleksji TASTy do dynamicznego tworzenia struktur typów, które następnie są materializowane jako kod bajtowy JVM.

\subsubsection{Konstrukcja symbolu klasy}\label{subsubsec:konstrukcja-symbolu-klasy}

Anonimowa klasa implementująca \texttt{Tokenization[Ctx]} jest tworzona poprzez wywołanie \texttt{Symbol.newClass} (linie 201-207 w~\texttt{Lexer.scala}):

\lstinputlisting[language=scala,caption={Wywołanie \texttt{Symbol.newClass} do utworzenia symbolu klasy anonimowej},label={lst:lexer-13-symbol-newclass}, linerange={201-207}]{../../src/alpaca/lexer/Lexer.scala}
...
\begin{itemize}
    ...
    \item \textbf{None} — opcjonalna klasa companion (niewykorzystana w~tym przypadku)
\end{itemize}

\subsubsection{Definicja członków klasy}\label{subsubsec:definicja-czlonkow-klasy}

Funkcja \texttt{decls} (linie 143-199) konstruuje pełną listę deklaracji dla klasy anonimowej:
...
\subsubsection{Materiałizacja klasy}\label{subsubsec:materializacja-klasy}
...
Pełny proces generacji i~instancjowania przedstawia listing~\ref{lst:lexer-14-anon-class}.
...
Mechanizm typów rafinowanych stanowi fundamentalną cechę systemu typów Scali umożliwiającą precyzyjne wyrażenie struktury typów w~czasie kompilacji.
...
\lstinputlisting[language=scala,caption={Rafinowanie typu wynikowego o pola tokenów},label={lst:lexer-15-refinements}, linerange={267-276}]{../../src/alpaca/lexer/Lexer.scala}

Funkcja \texttt{Refinement(tpe, name, memberType)} tworzy nowy typ będący rozszerzeniem typu \texttt{tpe} o~członka \texttt{name} typu \texttt{memberType}.
Operacja ta jest wykonywana w~czasie kompilacji i~nie generuje dodatkowego kodu w~czasie wykonania.

\subsubsection{Typ przecięcia (intersection type)}\label{subsubsec:typ-przeciecia}
...
Ten typ reprezentuje wartości będące jednocześnie instancjami \texttt{Tokenization[Ctx]} oraz posiadające określone pola strukturalne.
Kompilator Scali wykorzystuje tę informację do:
\begin{itemize}
    \item Walidacji dostępu do pól w~czasie kompilacji
    \item Zapewnienia inferowania precyzyjnych typów
    \item Umożliwienia refaktoryzacji z~pełnym wsparciem IDE
\end{itemize}
...
Wszystkie definicje tokenów są rozwiązywane statycznie w~czasie kompilacji.
Dostęp do tokenów odbywa się poprzez bezpośrednie odwołanie do pola klasy, co po kompilacji do kodu bajtowego JVM redukuje się do instrukcji \texttt{getfield} — operacji o~złożoności O(1) bez żadnego narzutu pośrednictwa.
...
Zastosowanie typów rafinowanych łączy zalety typowania strukturalnego (elastyczność w~dostępie do składowych) z~bezpieczeństwem typowania nominalnego (jednoznaczna identyfikacja typów).
...
\subsection{Podsumowanie korzyści}\label{subsec:podsumowanie-korzysci-anon-class}

Zastosowanie klas anonimowych i~typów rafinowanych w~implementacji systemu ALPACA zapewnia kompleksowe rozwiązanie łączące w~sobie zaawansowane techniki metaprogramowania z~pragmatycznymi rozwiązaniami inżynierskimi.

Tabela~\ref{tab:benefits-anon-class} przedstawia syntetyczne zestawienie korzyści wynikających z~przyjętego podejścia implementacyjnego.

\begin{table}[h]
\centering
\caption{Korzyści wynikające z zastosowania klas anonimowych i typów rafinowanych}
\label{tab:benefits-anon-class}
\begin{tabular}{|l|p{8cm}|}
\hline
\textbf{Aspekt} & \textbf{Korzyść} \\
\hline
...
System dostarcza rozwiązanie o~wysokim poziomie abstrakcji przy zachowaniu maksymalnej wydajności i~bezpieczeństwa.
