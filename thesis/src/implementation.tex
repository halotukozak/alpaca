...
System udostępnia interfejs języka dziedzinowego (DSL) oparty na dopasowaniu wzorców, umożliwiająci deklaratywne wyrażenie reguł tokenizacji.
...
W~naszym podejściu makro generuje definicję \verb|type Fields| zawierającą wszystkie zdefiniowane tokeny i~ich typy, dzięki czemu udaje się uzyskać następujące rezultaty:
\begin{itemize}
...
Alternatywne podejście oparte na strukturze mapy wymagałoby następujących elementów.
\begin{enumerate}
...
\noindent Do wad tego podejścia należą następujące aspekty.
\begin{itemize}
...
\noindent Główne wady tego podejścia są następujące.
\begin{itemize}
...
Realizacja napotkała szereg wyzwań technicznych, z~których najistotniejsze to następujące problemy.
\begin{itemize}
...
W tym podejściu realizowane są następujące kroki.
\begin{enumerate}
...
Akcje semantyczne definiowane przez użytkownika mogą odwoływać się do następujących elementów.
\begin{itemize}
...
\noindent Kluczowe aspekty implementacji obejmują następujące punkty.
\begin{enumerate}
...
Implementacja wykorzystuje klasę \verb|ConflictResolutionTable|, która podczas konstrukcji tabeli parsowania wykonuje następujące czynności.
\begin{enumerate}
...
Standardowe podejście wymagałoby podjęcia następujących działań.
\begin{itemize}
...
Mechanizm derywacji wymaga spełnienia następujących warunków.
\begin{itemize}
...
\noindent Klasa~\verb|ReplaceRefs| implementuje metodę~\verb|transformTree|, która realizuje następujące zadania.
\begin{enumerate}
...
\noindent Klasa~\verb|CreateLambda| implementuje algorytm konstrukcji wyrażeń lambda poprzez następujące kroki.
\begin{enumerate}
