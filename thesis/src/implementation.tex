...
Klasa typu \texttt{Empty[T]} umożliwia tworzenie pustych instancji typów produktowych (ang. product type) poprzez wykorzystanie wartości domyślnych parametrów konstruktora.
...
Istnieje automatyczna derywacja dla dowolnego typu produktowego z parametrami domyślnymi.
Ta derywacja oparta na makrze wykorzystuje domyślne wartości parametrów konstruktora do stworzenia mechanizmu tworzenia instancji dla danego typu.
...
Klasa \texttt{CreateLambda} służy do tworzenia wyrażeń lambda podczas ekspansji makr.
...
val lambdaExpr: Expr[Int => Int] = createLambda[Int => Int] {
  case (sym, List(arg)) =>
    // Budowanie ciała drzewa z wykorzystaniem symbolu metody i drzew argumentów
    buildBody(arg)
}
\end{lstlisting}
...
Istnieje automatyczna derywacja dla dowolnego typu produktowego (klasy przypadku) z wykorzystaniem \texttt{Mirror.ProductOf}.
