...
\section[Praktyczna implementacja analizatora leksykalnego z wykorzystaniem makr w Scali 3]{Praktyczna implementacja analizatora leksykalnego z~wykorzystaniem makr w~Scali~3}\label{sec:praktyczna-implementacja-analizatora-leksykalnego-z-wykorzystaniem-makr-w-scali-3}
...
\subsection{Interfejs użytkownika}\label{subsec:interfejs-uzytkownika}
...
\lstinputlisting[language=scala,caption={Definicja typu LexerDefinition},label={lst:lexer-definition}]{listings/implementation/lexer-01-definition.scala}
...
\lstinputlisting[language=scala,caption={Punkt wejścia: transparent inline def lexer},label={lst:lexer-entrypoint}]{listings/implementation/lexer-02-entrypoint.scala}
...
\subsection{Implementacja makra}\label{subsec:implementacja-makra}
...
\subsection[Analiza i transformacja drzewa składni]{Analiza i~transformacja drzewa składni}\label{subsec:analiza-i-transformacja-drzewa-skladni}
...
\lstinputlisting[language=scala,caption={Dekonstrukcja funkcji częściowej (dopasowanie AST do CaseDef)},label={lst:extract-cases}]{listings/implementation/lexer-03-extract-case.scala}

Fragment ten wykorzystuje dopasowanie wzorców w~cytatach (ang.~\emph{quotes}) do dekonstrukcji~\cite{scala3-guides-quotes} typowanego AST funkcji częściowej.
...
\subsubsection[Transformacja i adaptacja referencji]{Transformacja i~adaptacja referencji}\label{subsubsec:transformacja-i-adaptacja-referencji}
...
\lstinputlisting[language=scala,caption={Zastąpienie referencji starego kontekstu nowymi (ReplaceRefs)},label={lst:replace-with-new-ctx}]{listings/implementation/lexer-04-replace-refs-with-new-ctx.scala}
...
\subsection[Ekstrakcja i kompilacja wzorców]{Ekstrakcja i~kompilacja wzorców}\label{subsec:ekstrakcja-i-kompilacja-wzorcow}
...
\lstinputlisting[language=scala,caption={Funkcja extractSimple: dopasowywanie definicji tokenów},label={lst:lexer-08-extract-simple}]{listings/implementation/lexer-05-extract-simple.scala}
...
Metoda \verb|Symbol.newClass| przyjmuje następujące parametry:
\begin{itemize}
...
Funkcja \verb|decls| konstruuje pełną listę deklaracji dla klasy anonimowej:
...
\lstinputlisting[language=scala,caption={Rafinowanie typu wynikowego o~pola tokenów},label={lst:refinements}]{listings/implementation/lexer-06-refinements.scala}
...
\lstinputlisting[language=scala,caption={Wynikowy typ leksera},label={lst:lexer-15-result-type}]{listings/implementation/lexer-15-result-type.scala}
...
\lstinputlisting[language=scala,caption={Tworzenie typuFields},label={lst:fields}]{listings/implementation/lexer-07-fields.scala}

\subsection{Uzasadnienie wybranego podejścia implementacyjnego}\label{subsec:uzasadnienie-wybranego-podejscia}

\subsubsection[Eliminacja narzutu wykonania w czasie działania programu]{Eliminacja narzutu wykonania w~czasie działania programu}\label{subsubsec:eliminacja-narzutu-wykonania}
...
Alternatywne podejście oparte na strukturze mapującej (np.\ \verb|Map[String, Token]|) wymagałoby:
\begin{itemize}
...
\subsubsection[Integracja z narzędziami deweloperskimi]{Integracja z~narzędziami deweloperskimi}\label{subsubsec:integracja-z-narzedzimi}
...
\subsubsection[Typowanie strukturalne z gwarancjami nominalnymi]{Typowanie strukturalne z~gwarancjami nominalnymi}\label{subsubsec:typowanie-strukturalne}
...
\lstinputlisting[language=scala,caption={Podejście oparte na mapowaniu dynamicznym},label={lst:lstlisting-dynamic-mapping}]{listings/implementation/lexer-12-dynamic-mapping.scala}
...
\lstinputlisting[language=scala,caption={Podejście oparte na jawnej definicji klasy},label={lst:lstlisting-explicit-lexer}]{listings/implementation/lexer-13-explicit-lexer.scala}
...
\subsection[Walidacja i obsługa błędów]{Walidacja i~obsługa błędów}\label{subsec:walidacja-i-obsuga-bedow}
...
Metoda \verb|report.errorAndAbort| przerywa kompilację i~wyświetla komunikat o~błędzie, eliminując konieczność detekcji błędów w~czasie wykonania, co jest zgodne z~zasadą wczesnej walidacji (\textit{fail-fast})~\cite{scala3-reference-macros,scala3-guides-macros}.
...
Centralnym elementem systemu jest makro \verb|createTablesImpl|, które analizuje definicję parsera i generuje tabele w~czasie kompilacji:
...
Wymaga to zastosowania techniki \textit{re-owning} symboli, realizowanej przez klasę \verb|ReplaceRefs|.
...
\lstinputlisting[language=scala,caption={Naiwna implementacja prowadząca do przekroczenia limitu},label={lst:parser-05-naive}]{listings/implementation/parser-05-naive.scala}
...
Dodatkowo kompilator JIT może efektywnie zoptymalizować te~metody poprzez \textbf{inlining}, eliminując narzut wywołań funkcji w~czasie wykonania.
...
\begin{enumerate}
  ...
  \item System typów zapewnia, że ekstrakcje są bezpieczne względem typów dzięki informacji z wzorca dopasowania
\end{enumerate}
...
\lstinputlisting[language=scala,caption={Deklaracja rozwiązań konfliktów},label={lst:parser-08-conflicts}]{listings/implementation/parser-08-conflicts.scala}
...
Implementacja \verb|ToExpr[ParseTable]| jest szczególnie interesująca, gdyż musi radzić sobie z~potencjalnie dużymi tabelami (patrz~\ref{subsubsec:problem-ograniczenia-rozmiaru-metod-jvm}):
...
\begin{itemize}
  ...
  \item \verb|Copyable[T]| — generyczna funkcja kopiowania dla klas przypadku (\textit{case classes}).
\end{itemize}
...
Klasa typu~\verb|Empty[T]| stanowi abstrakcję nad mechanizmem konstrukcji wartości domyślnych dla typów produktowych (ang.~\textit{product types})~\cite{pierce2002}.
W~systemie typów Scali~\cite{odersky2004overview} typy produktowe odpowiadają klasom przypadku (\textit{case classes}) oraz krotkom (\textit{tuples}), będącym reprezentacją iloczynów kartezjańskich typów składowych.
...
\lstinputlisting[language=scala,caption={Definicja klasy typu Empty[T]},label={lst:empty}]{listings/implementation/lexer-08-empty.scala}

Typ~\verb|Empty[T]| jest reprezentowany jako funkcja zerargumentowa~\verb|() => T|, co~umożliwia leniwą inicjalizację instancji (\textit{lazy instantiation}). Atrybut~\verb|private[alpaca]| ogranicza widoczność do~pakietu, zapobiegając przypadkowemu użyciu poza systemem.

\subsubsection{Mechanizm derywacji}
...
\lstinputlisting[language=scala,caption={Użycie Empty[T]},label={lst:empty-usage}]{listings/implementation/lexer-09-empty-usage.scala}

\subsubsection[Alternatywy i ich ograniczenia]{Alternatywy i~ich ograniczenia}
Bez mechanizmu~\verb|Empty[T]| konstrukcja wartości domyślnej w~kontekście generycznym wymagałaby od użytkownika jawnego podania domyślnych wartości dla każdego typu~\verb|T|, co~wyeliminowałoby zalety programowania generycznego.

Mechanizm~\verb|Empty[T]| eliminuje te problemy poprzez derywację w~czasie kompilacji, zachowując bezpieczeństwo typów oraz zerowy narzut wykonania.
...
\subsection[ReplaceRefs — transformacja symboli w AST]{ReplaceRefs — transformacja symboli w~AST}\label{subsec:replacerefs}
...
Klasa~\verb|TreeMap| definiuje wzorzec projektowy odwiedzającego (\textit{visitor pattern})~\cite{gamma1994} dla typowanego AST Scali~3, umożliwiając rekurencyjne przetwarzanie węzłów drzewa z~zachowaniem bezpieczeństwa typów.
...
Proces ten, znany jako \textit{re-owning}~\cite{scala3-guides-reflection}, wymaga systematycznej zamiany wszystkich referencji do~starego symbolu nowymi referencjami.

\subsubsection{Definicja}

\lstinputlisting[language=scala,caption={Definicja klasy ReplaceRefs rozszerzającej TreeMap},label={lst:utils-03-replacerefs}]{listings/implementation/utils-03-replacerefs.scala}

\subsubsection{Mechanizm działania}
Klasa~\verb|ReplaceRefs| implementuje metodę~\verb|transformTree|, która:
\begin{enumerate}
...
\subsubsection{Przykład użycia}
Poniższy fragment ilustruje zastąpienie referencji do~parametru makra~\verb|oldCtx| nowym symbolem~\verb|newCtx| w~ciele wygenerowanej metody:

\lstinputlisting[language=scala,caption={Użycie ReplaceRefs w~kontekście ekspansji makra},label={lst:utils-04-replacerefs-usage}]{listings/implementation/utils-04-replacerefs-usage.scala}
...
\lstinputlisting[language=scala,caption={Definicja klasy CreateLambda do~programatycznej konstrukcji wyrażeń lambda},label={lst:utils-05-createlambda}]{listings/implementation/utils-05-createlambda.scala}

\subsubsection{Mechanizm działania}
Klasa~\verb|CreateLambda| implementuje algorytm konstrukcji wyrażeń lambda poprzez:
\begin{enumerate}
...
\lstinputlisting[language=scala,caption={Użycie CreateLambda do~konstrukcji wyrażenia funkcyjnego},label={lst:utils-06-createlambda-usage}]{listings/implementation/utils-06-createlambda-usage.scala}

\subsection{Copyable[T] — generyczna funkcja kopiowania}\label{subsec:copyable}

Klasa typu~\verb|Copyable[T]| definiuje operację kopiowania (\textit{shallow copy}) dla typów produktowych.
W~systemie~\textit{ALPACA} kopiowanie jest wykorzystywane do~tworzenia nowych instancji kontekstu parsera z~modyfikowanymi polami (np.~aktualizacja numeru wiersza po~napotkaniu znaku nowej linii), bez konieczności ręcznej rekonstrukcji całej struktury.
...
\lstinputlisting[language=scala,caption={Definicja klasy typu Copyable[T]},label={lst:copyable}]{listings/implementation/lexer-10-copyable.scala}
...
\lstinputlisting[language=scala,caption={Użycie Copyable[T]},label={lst:copyable-usage}]{listings/implementation/lexer-11-copyable-usage.scala}

\subsubsection{Uwaga techniczna}
Operacja kopiowania realizowana przez~\verb|Copyable[T]| jest kopią płytką (\textit{shallow copy}) — pola będące referencjami do~obiektów wskazują na~te same instancje w~oryginalnej i~skopiowanej strukturze.
...
W~kontekście systemu~\textit{ALPACA} korzyść z~unifikacji interfejsu (klasa typu) przewyższa potencjalne różnice wydajnościowe, które są pomijalnie małe dla operacji kopiowania struktur o~niewielkim rozmiarze (kilka pól).

\subsection[Porównanie z istniejącymi bibliotekami]{Porównanie z~istniejącymi bibliotekami}\label{subsec:porownanie-narzedzi}
...
Wybór własnej implementacji narzędzi~\verb|Empty[T]| i~\verb|Copyable[T]| w~systemie~\textit{ALPACA} wynikał z~następujących przesłanek:
...
Własna implementacja pozwala dostosować komunikaty błędów do~kontekstu systemu~\textit{ALPACA}.
