...
Przykład~\ref{lst:review-python} ilustruje kilka nieintuicyjnych, automatycznych mechanizmów obecnych w~bibliotece \textit{SLY}.
...
Komunikaty błędów w~bibliotece \textit{SLY} nie zawierają informacji o~kontekście syntaktycznym ani sugestii poprawek, co obrazuje przykład~\ref{lst:review-sly-err}, który po uruchomieniu informuje użytkownika błędem z~fragmentu kodu~\ref{lst:review-sly-out}.
...
\lstinputlisting[language=terminal,caption={Przykładowy komunikat błędu w~bibliotece \textit{SLY}},label={lst:review-sly-out}]{listings/introduction/intro-04-not-working-result}
