...
Analizatory leksykalne (ang.~\emph{lexers}) i~składniowe (ang.~\emph{parsers}) są fundamentalnymi komponentami procesu kompilacji, realizując kolejno fazę analizy leksykalnej i~syntaktycznej~\cite{aho2006}.
...
Parsery dla CFG dzielą się na dwie główne kategorie: parsery zstępujące (ang.~\emph{top-down}), takie jak LL(\(k\))~\cite{lewis1968}, oraz parsery wstępujące (ang.~\emph{bottom-up}), takie jak LR(\(k\))~\cite{knuth1965}.
...
\begin{enumerate}
    ...
    \item \textbf{Natywną integrację ze~środowiskami IDE} --- implementacja w~czystym języku Scala eliminuje konieczność stosowania dedykowanych pluginów, wykorzystując istniejące wsparcie dla języka (IntelliJ IDEA, Metals).
\end{enumerate}
