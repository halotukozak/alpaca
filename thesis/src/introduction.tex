...
\noindent W~ramach weryfikacji tezy sformułowano następujące pytania badawcze:
\begin{itemize}
...
Celem pracy jest zaprojektowanie i~zaimplementowanie narzędzia \emph{ALPACA} (\emph{Another Lexer Parser And Compiler Alpaca}) w~języku Scala, które implementuje funkcjonalności powszechnie stosowane w~budowie analizatorów leksykalnych i~składniowych, weryfikując postawioną tezę.
...
Identyfikacja tych ograniczeń stanowiła punkt wyjścia dla projektu~\emph{ALPACA}.

Projekt~\emph{ALPACA} stanowi narzędzie do~generowania lekserów i~parserów w~języku Scala, łączące zalety istniejących rozwiązań poprzez:
...
\label{subsec:review-generators}
...
\label{subsubsec:review-lex-yacc}

\emph{Lex}~\cite{lesk1975lex} i~\emph{Yacc}~\cite{johnson1975yacc} to klasyczne, dobrze ugruntowane narzędzia, które odegrały kluczową rolę w~tworzeniu setek współczesnych języków programowania.
...
Ponieważ \emph{Lex} i~\emph{Yacc} zostały zaprojektowane do współpracy z~językiem~C, ich integracja z~nowoczesnymi językami programowania bywa utrudniona.
...
\lstinputlisting[language=c,caption={Fragment definicji parsera Ruby z~wykorzystaniem technologii Yacc.},label={lst:review-ruby}]{listings/introduction/intro-01-ruby-parser.y}

\subsubsection{ANTLR}
\label{subsubsec:review-antlr}

\emph{ANTLR}~\cite{parr2004s} to rozwiązanie inspirowane narzędziami \emph{Lex} i~\emph{Yacc}, oferujące zaawansowane mechanizmy analizy składniowej.
...
Na~podstawie tej definicji \emph{ANTLR} generuje parser w~wybranym przez użytkownika języku programowania, takim jak Python, Java, C++ lub JavaScript.

Wspomaganie pracy z~\emph{ANTLR} w~znacznym stopniu ułatwiają dedykowane wtyczki do środowisk Visual Studio Code oraz IntelliJ IDEA\@.
...
Jedną z~kluczowych różnic \emph{ANTLR} w~porównaniu do innych narzędzi jest wykorzystanie gramatyki LL(*), podczas gdy klasyczne rozwiązania, takie jak Yacc czy SLY, implementują LALR(1).
...
Dodatkowym wyzwaniem podczas korzystania z~\emph{ANTLR} jest konieczność nauki składni DSL Grammar~v4 oraz ograniczenie wsparcia dla narzędzi deweloperskich.
Pełne wykorzystanie możliwości \emph{ANTLR} wymaga korzystania z~jednego z~dedykowanych środowisk, co może stanowić istotne ograniczenie dla użytkowników preferujących inne IDE\@.

\subsection{Biblioteki interpretowane}
\label{subsec:review-interpreted}
...
\label{subsubsec:review-ply-sly}

\emph{PLY}~\cite{ply} i~jego nowszy odpowiednik \emph{SLY}~\cite{sly} to biblioteki inspirowane narzędziami Lex i~Yacc.
...
Mechanizm refleksji wykorzystywany przez bibliotekę~\emph{SLY} (inspekcja nazw metod i~typów) powoduje generowanie ostrzeżeń przez analizatory statyczne środowiska PyCharm.
Ponadto należy zaznaczyć, iż autor projektu informuje o~braku dalszego rozwoju tych narzędzi~\cite{sly-github}.

Przykład~\ref{lst:review-python} ilustruje kilka nieintuicyjnych, automatycznych mechanizmów obecnych w~bibliotece \emph{SLY}.

Dekorator \texttt{@\_()} definiuje wzorzec dopasowania dla produkcji.
...
Konwencja nazewnictwa metod określa typ zwracany przez produkcję.
...
W~krotce \verb|precedence| definiowane jest pierwszeństwo operatorów, jednakże dodanie \verb|\%~prec| pozwala nadpisać priorytet dla konkretnej reguły składniowej.

Argument~\verb|p| pozwala na dostęp do~kontekstu produkcji (np.\ numeru linii), ale także do zmiennych we wzorcu dopasowania w~adnotacji.
Jeśli zdefiniowany jest więcej niż jeden element, dodawany jest numer do akcesora, np.~\verb|expr1| jest odwołaniem do drugiego wyrażenia \verb|expr|.

\lstinputlisting[language=Python,caption={Fragment definicji parsera w~Pythonie, wykorzystujący bibliotekę SLY.},label={lst:review-python}]{listings/introduction/intro-02-sly-parser.py}

Komunikaty błędów w~bibliotece \emph{SLY} nie zawierają informacji o~kontekście syntaktycznym ani sugestii poprawek, co obrazuje przykład~\ref{lst:review-sly-err}, który po uruchomieniu informuje użytkownika błędem z~fragmentu kodu~\ref{lst:review-sly-out}.
Okazuje się, że problemem był brak atrybutu \verb|ignore\_comment| w~definicji \verb|Lexer|.

\lstinputlisting[language=Python,caption={Fragment niedziałającego kodu w~Pythonie, wykorzystujący bibliotekę SLY.},label={lst:review-sly-err}]{listings/introduction/intro-03-not-working-sly.py}

\lstinputlisting[language=terminal,caption={Przykładowy komunikat błędu w~bibliotece \emph{SLY}.},label={lst:review-sly-out}]{listings/introduction/intro-04-not-working-result}

\subsection{Kombinatory parserów}
\label{subsec:review-combinators}
...
\label{subsubsec:review-scala-pc}

Biblioteka \emph{Scala parser combinators}~\cite{moors2008parser} była popularnym sposobem na tworzenie parserów, lecz jak stwierdzono w~samej dokumentacji: \enquote{Trudno jest jednak zrozumieć ich działanie i~jak zacząć.
...
\label{subsubsec:review-scala-bison}

Z~podsumowania artykułu na temat \emph{ScalaBison}~\cite{boyland2010tool} wiadomo, że to praktyczny generator parserów dla języka Scala oparty na technologii rekurencyjnego wstępowania i~zstępowania, który akceptuje pliki wejściowe w~formacie \emph{bison}.
Parsery generowane przez \emph{ScalaBison} używają bardziej informacyjnych komunikatów o~błędach niż te generowane przez pierwowzór \emph{bison}, a~także szybkość parsowania i~wykorzystanie miejsca są znacznie lepsze niż \emph{scala-combinators}, ale są nieco wolniejsze niż najszybsze generatory parserów oparte na JVM\@.

Dodatkowo należy zaznaczyć, iż jest to rozwiązanie już niewspierane i~stworzone w~celach akademickich.
Korzysta z~przestarzałej wersji Scali, nie posiada wyczerpującej dokumentacji i~liczba funkcjonalności jest bardzo ograniczona w~porównaniu do np.~technologii \emph{SLY}.

\subsubsection{parboiled2}
\label{subsubsec:review-parboiled2}

\emph{parboiled2}~\cite{myltsev2019parboiled2} to biblioteka w~Scali umożliwiająca lekkie i~szybkie parsowanie dowolnego tekstu wejściowego.
...
Zgodnie z~przykładem~\ref{lst:parboiled2-error}, komunikaty błędów nie zawierają informacji o~kontekście syntaktycznym oraz nie sugerują możliwych poprawek (problem z~implementacją wynika jedynie z~różnic w~liczbie parametrów funkcji).
...
Niewielki fragment (14 z~133~linii) błędu wygenerowanego przez bibliotekę \emph{parboiled2}, który pochodzi z~prezentacji Li~Haoyi na temat \emph{FastParse}~\cite{fastparse-talk}.
},label={lst:parboiled2-error}]{listings/introduction/intro-05-parboiled2-error}

\subsubsection{FastParse}
\label{subsubsec:review-fastparse}

\emph{FastParse}~\cite{fastparse-docs} to opracowana przez Li~Haoyi wysokowydajna biblioteka kombinatorów parserów dla Scali, zaprojektowana w~celu uproszczenia tworzenia parserów tekstu strukturalnego.
...
Według dokumentacji~\cite{fastparse-docs}, parsery \emph{Fastparse} zajmują 1/10 kodu w~porównaniu do ręcznie napisanego parsera rekurencyjnego.
W~porównaniu do narzędzi generujących parsery, takich jak \emph{ANTLR} lub \emph{Lex} i~\emph{Yacc}, implementacja nie wymaga żadnego specjalnego kroku kompilacji lub generowania kodu.
To sprawia, że rozpoczęcie pracy z~\emph{Fastparse} jest znacznie łatwiejsze niż w~przypadku bardziej tradycyjnych narzędzi do generowania parserów.
...
Tabela~\ref{tab:review-comparison} zestawia główne cechy analizowanych narzędzi.
...
\noindent Żadne z~analizowanych rozwiązań nie łączy jednocześnie:
\begin{itemize}
...
Luka ta stanowi motywację dla projektu~\emph{ALPACA}, który wykorzystuje makra kompilacyjne Scali~3 do osiągnięcia tych celów jednocześnie.

\begin{table}[ht]
    \centering
    \begin{tabularx}{\textwidth}{X|*{4}{Y}}
        \toprule
