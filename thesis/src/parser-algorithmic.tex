...
\subsection[Gramatyki bezkontekstowe i klasy parserów]{Gramatyki bezkontekstowe i~klasy parserów}\label{subsec:grammar-parser-classes}
...
\section[Dobór klasy parsera w systemie ALPACA]{Dobór klasy parsera w~systemie ALPACA}\label{sec:parser-lr1-choice}
...
\subsection[Zalety podejścia LR(1) w ALPACA]{Zalety podejścia LR(1) w~ALPACA}\label{subsec:lr1-advantages}
...
\subsection[Koszty i ograniczenia]{Koszty i~ograniczenia}
...
\section[Konstrukcja tabel parsera LR(1) w ALPACA]{Konstrukcja tabel parsera LR(1) w~ALPACA}
...
\begin{itemize}
    ...
    Jeśli~stan docelowy już~istnieje, do~tabeli akcji dodawana jest~akcja \textit{shift} do~jego ID; w~przeciwnym razie stan otrzymuje nowe ID, zostaje dodany do~listy stanów, a~\textit{shift} wskazuje na~nowy stan.
    \item Elementy z~kropką na~końcu produkcji (produkcja całkowicie rozpoznana) dodają akcje \textit{reduce} dla~swoich prefiksów wejścia.
    Specjalny element \texttt{S'~→~root~•,~\$} generuje akcję \textit{accept} dla~symbolu \texttt{\$}.
...
\label{subsec:parser-conflict-resolution}

Podczas konstrukcji tabel parsera LR(1) mogą wystąpić sytuacje, w~których dla~tej samej pary (stan, symbol) istnieje więcej niż~jedna możliwa akcja---konflikty \textit{shift/reduce} lub~\textit{reduce/reduce}.
...
\label{subsubsec:basic-precedence}
...
Podczas budowy tabeli parsera, gdy~algorytm napotyka konflikt między~akcjami~$a_1$ i~$a_2$:
...
W~przypadku braku reguły precedencji kompilator rzuca wyjątek zawierający:
\begin{itemize}
    ...
    \item Sugestię użycia mechanizmu \texttt{before}/\texttt{after}
\end{itemize}

\subsubsection{Przechodniość przez rozwinięcie grafu relacji}
\label{subsubsec:transitive-graph}
...
Dzięki temu rozwiązaniu użytkownik może zadeklarować jedynie podstawowe reguły precedencji (np.~\textit{mnożenie}~$>$~\textit{dodawanie}), a~system automatycznie wyprowadzi wszystkie implikacje przechodnie.
...
\paragraph{Przykład}
Dla~gramatyki wyrażeń arytmetycznych z~reguł:
\begin{align*}
...
\lstinputlisting[language=scala,caption={Implementacja rozwiązywania konfliktów z~przechodniością},label={lst:conflict-resolution-get},escapeinside={(*@}{@*)}]{listings/chapter5/conflict-resolution-get.scala}

\subsubsection{Weryfikacja acykliczności w czasie kompilacji}
\label{subsubsec:acyclicity-verification}

Istotnym wymogiem poprawności systemu precedencji jest brak cykli w~grafie relacji.
Obecność cyklu oznaczałaby niespójność, w~której produkcja~\textit{A} miałaby pierwszeństwo nad~\textit{B}, \textit{B}~nad~\textit{C}, a~jednocześnie \textit{C}~nad~\textit{A}---sytuację nie~do~rozstrzygnięcia.
...
\lstinputlisting[language=scala,caption={Implementacja weryfikacji acykliczności grafu precedencji},label={lst:conflict-resolution-verify}]{listings/chapter5/conflict-resolution-verify.scala}

\paragraph{Kluczowe właściwości}
\begin{itemize}
    \item Jeśli~algorytm zakończy się~bez~wyjątku, graf jest~acykliczny (DAG)
...
Kompilator rzuca wyjątek \texttt{InconsistentConflictResolution} zawierający:
\begin{itemize}
...
Jest~to kluczowy element zasady \textit{fail-fast}, która maksymalizuje bezpieczeństwo i~przewidywalność systemu.
