...
Parser przekształca strumień tokenów w strukturę danych (zazwyczaj jest to drzewo składniowe), rozstrzygając zgodność wejścia z gramatyką. Klasyczne podejścia bazują na automatach skończonych rozszerzonych o stos (PDA)\cite{pushdown-automaton-wiki} oraz na algorytmach predykcyjnych\cite{predictive-parsing} lub analizie przesuwająco-redukcyjnej\cite{shift-reduce-parser}.

\subsection{Gramatyki i typy parserów}
Gramatyka bezkontekstowa (CFG) składa się z nieterminali, terminali (tokenów), symbolu startowego i produkcji. Wśród klas parserów wyróżnia się:
\begin{itemize}
    \item \textbf{LL(k)}\cite{ll-parser-wiki} (zstępujące, predykcyjne) — konstruują lewostronne wyprowadzenia, wybierając produkcje na podstawie prefiksu wejścia (lookahead). Wymagają gramatyk bez lewostronnej rekurencji i z niekolidującymi zbiorami FIRST/FOLLOW.
    \item \textbf{LR(k)}\cite{lr-parser-wiki} (wstępujące, shift-reduce) — rekonstruują prawostronne wyprowadzenia wstecz, używając stosu stanów automatu LR. Radzą sobie z większą klasą gramatyk, w tym lewostronnie rekurencyjnych.
\end{itemize}
...
Parsery tabelowe (LL i LR) działają w stałym czasie na token\cite{knuth1965lr}. Tabela akcji LR mapuje parę (stan, lookahead) na \textit{shift}, \textit{reduce}, \textit{accept} lub \textit{error}; tabela goto określa przejścia po redukcjach. W LL analogiczną rolę pełni tabela predykcji (nieterminal × lookahead → produkcja)\cite{ll-parser-wiki}.
...
Najczęściej rozwiązuje się je przez zmianę gramatyki lub zwiększenie lookaheadu\cite{illinois-parsing-conflicts}.
...
arytmetycznych.\cite{ll-parser-wiki}
...
Na bazie produkcji wyliczany jest \texttt{FirstSet} (iteracyjnie do punktu stałego)\cite{first-set-geeksforgeeks}, który określa możliwe terminale po kropce. Algorytm przechodzi po wszystkich produkcjach, dodając:
\begin{itemize}
    \item pierwszy terminal z prawej strony produkcji,
    \item w przypadku nieterminali — wszystkie terminale z ich FIRST (z wyjątkiem $\varepsilon$ - terminala oznaczającego pusty ciąg), a jeśli mogą generować $\varepsilon$, iteruje dalej po kolejnych symbolach produkcji,
...
budowane klasycznym schematem \textit{closure}/\textit{goto}\cite{agh-lr1-parsers} i deduplikowane, tak aby
