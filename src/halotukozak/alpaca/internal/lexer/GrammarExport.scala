...
// $COVERAGE-OFF$
  def maybeWrite(lexerName: String, tokens: List[(name: String, pattern: String, ignored: Boolean)])(
  ...
  private def toJson(tokens: List[(name: String, pattern: String, ignored: Boolean)]): String =
    tokens
      .map(t => s"""{"name":${JsonExport.quote(t.name)},"pattern":${JsonExport.quote(t.pattern)},"ignored":${t.ignored}}""")
      .mkString("[", ",", "]")
// $COVERAGE-ON$
