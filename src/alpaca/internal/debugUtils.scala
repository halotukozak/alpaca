...
import DebugPosition.given_is_DebugPosition_Showable
...
private[internal] object DebugPosition {
  given DebugPosition is Showable = (line, file) => s"$line ($file)"
...
extension (using quotes: Quotes)(tree: quotes.reflect.Tree)
  ...
  private[internal] def dbg(using pos: DebugPosition): tree.type = {
    quotes.reflect.report.errorAndAbort(show"${Showable.given_is_Tree_Showable.show(tree)} at line $pos")
    tree
  }
  ...
  private[internal] def info(using pos: DebugPosition): tree.type = {
    quotes.reflect.report.info(show"${Showable.given_is_Tree_Showable.show(tree)} at line $pos")
    tree
  }
