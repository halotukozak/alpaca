 ...
 * [[Regex.range]], [[Regex.chars]], [[Regex.alt]], [[Regex.inter]], [[Regex.all]]) and
...
private[lexer] object Regex:
  ...
  /** All-strings regex `Σ*`. */
  val all: Regex = chars(CharSet.all).star

  /** Convenience: literal char. */
  def lit(c: Char): Regex = Chars(CharSet.single(c))

  /** Convenience: char range. */
  def range(lo: Char, hi: Char): Regex = Regex(CharSet.range(lo, hi))

  /** Convenience: any single code point in `set`. Returns `Empty` if `set` is empty. */
  def chars(set: CharSet): Regex = Regex(set)
...
private[regex] object CharSet:
  ...
  def single(c: Int): CharSet =
    require(0 <= c && c <= maxCodePoint, s"code point out of range: $c")
    CharSet(Vector(Range(c, c)))
  def range(lo: Char, hi: Char): CharSet = range(lo.toInt, hi.toInt)
  def range(lo: Int, hi: Int): CharSet =
    require(0 <= lo && lo <= hi && hi <= maxCodePoint, s"invalid range [$lo, $hi]")
    CharSet(Vector(Range(lo, hi)))
...
private[regex] object Range:
  def apply(lo: Int, hi: Int): Range = (lo, hi)
  def apply(lo: Char, hi: Char): Range = (lo.toInt, hi.toInt)
