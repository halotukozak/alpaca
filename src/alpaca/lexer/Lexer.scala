...
import alpaca.*
import alpaca.core.{CreateLambda, ReplaceRefs, matchOption, raiseShouldNeverBeCalled, given}
...
transparent inline def lexer[Ctx <: EmptyCtx](inline rules: Ctx ?=> LexerDefinition[Ctx]): Tokenization[Ctx] =
  ${ lexerImpl[Ctx]('{ rules }) }
