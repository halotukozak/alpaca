...
final class RegexParserTest extends AnyFunSuite with Matchers:
  ...
  test("parses dot") {
    parse(".") shouldBe Regex.chars(CharSet.dotDefault)
  }
  ...
  test("parses negated character class") {
    parse("[^a-z]") shouldBe Regex.chars(CharSet.range('a', 'z').complement)
  }

  test("parses character class with multiple ranges") {
    parse("[a-zA-Z0-9_]") shouldBe Regex.chars(
      CharSet.normalize(
        Range('a', 'z'),
        Range('A', 'Z'),
        Range('0', '9'),
